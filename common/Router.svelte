...
<div class='w-full h-full position-absolute overflow-hidden' class:invisible={!($media && (Object.keys($media).length > 0)) || (playPage && overlay.includes('viewanime')) || (!visible && (page !== 'player'))}>
  <Miniplayer active={($media && (Object.keys($media).length > 0)) && ((page !== 'player' && visible) || (overlay.includes('viewanime') && visible))} class='bg-dark-light rounded-10 z-100 {(page === `player` && !overlay.includes(`viewanime`)) ? `h-full` : ``}' padding='2rem'>
