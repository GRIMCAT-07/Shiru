...
<div class='overflow-hidden status-bar h-0' class:offline={$status === 'offline'}>
  <div class='z-101 position-absolute w-full d-flex align-items-center justify-content-center overflow-hidden status-bar h-0' class:offline={$status === 'offline'} class:bg-very-dark={$status === 'offline'} class:bg-success={$status !== 'offline'}>
    {#if $status === 'online'}
