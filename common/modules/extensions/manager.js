...
class ExtensionManager {
        ...
        settings.subscribe(value => {
            if (sources !== value.sourcesNew) {
                ...
                if (!Object.values(value.sourcesNew || {})?.length && this.defaultSource) this.whenReady = this.addSource(this.defaultSource)
                    ...
                    debug('Found new sources and updated...', value.sourcesNew)
                }
            }
        })
    ...
    async addSource(url) {
        ...
        if (!config) return `Failed to load extension(s) from the provided source '${url}': ${status.value !== 'offline' ? 'the source is not valid.' : 'no network connection!'}`
        ...
        settings.update((value) => {
            ...
            config.forEach(extension => {
                sourcesNew[extension.id] = extension
    ...
    async loadExtensions(extensions, update) {
        ...
        const cachedModules = !update && await Promise.all(extensionIds.map((id) => cache.cachedEntry(caches.EXTENSIONS, (extensions[id]?.locale || (extensions[id]?.update + '/')) + extensions[id]?.id, true)))

        const modules = {}
        for (const [index, extensionId] of extensionIds.entries()) {
            const cachedModule = cachedModules?.[index]
            const key = (extensions[extensionId]?.locale || (extensions[extensionId]?.update + '/')) + extensions[extensionId]?.id
            if (cachedModule) modules[key] = cachedModule
            else {
                const newCode = await getExtension(extensions[extensionId]?.name || extensions[extensionId]?.id, (extensions[extensionId]?.locale || (extensions[extensionId]?.update + '/')) + extensions[extensionId]?.main)
                modules[key] = (newCode && !extensions[extensionId].locale ? await cache.cacheEntry(caches.EXTENSIONS, key, { mappings: true }, newCode, Date.now() + getRandomInt(7, 14) * 24 * 60 * 60 * 1000) : newCode) || cache.cachedEntry(caches.EXTENSIONS, key, true)
                if (!modules[key]) continue
            }

            if (!this.activeWorkers[key]) {
                try {
                    if (this.inactiveWorkers[key]) this.inactiveWorkers[key].terminate()
                    const worker = createWorker(extensions[extensionId])
                    try {
    ...
    async updateExtensions(currentExtensions) {
        ...
        const latestValid = Object.fromEntries(latestManifests.flat().filter(config => this.validateConfig(config) && extensionIds.includes(config.id)).map(config => [config.id, config]))
        const toUpdate = extensionIds.filter(id => latestValid[id]?.version !== currentExtensions[id]?.version)
        if (toUpdate.length) {
            settings.update((value) => {
                const sourcesNew = { ...value.sourcesNew }
                toUpdate.forEach(id => {
                    try {
                        const key = (latestValid[id]?.locale || (latestValid[id]?.update + '/')) + id
                        if (this.activeWorkers[key] && latestValid[id]) {
                            this.activeWorkers[key].terminate()
                            delete this.activeWorkers[key]
                        }
                    } catch (error) {
